...
export default function Rolepermission() {
  ...
  const columns = [
    ...
    { field: 'description', headerName: 'Description', width: 240, align: 'left', headerAlign: 'left', editable: false },
    { field: 'permissions', headerName: 'Role', width: 140, align: 'left', headerAlign: 'left', editable: false,  renderCell: (params) => ( params.row.permissions.length )},
      ...
      getActions: (params) => {
        ...
        return [
          <GridActionsCellItem
            ...
            onClick={()=>{navigate(`/rolepermission/update/${params.id}`)}}
            color="inherit"
          />,
          <GridActionsCellItem
            key={index}
  ...
  const fetchWithParams = async (data) => {
    ...
    const response = await getRequest("auth/role",paramsRequest);
    if (response.data) {
        setData(response.data);
    }
    setLoading(false)
  ...
  const deleteData = async (id) => {
    setLoading(true);
    const response = await delRequest(`auth/role/${id}`);
    if (response.data){
      fetchData()
    }
    setLoading(false)
